...
def num_status_1(df: pd.DataFrame):

    df_a = df.groupby(["userId", "status", "week_slice"]).size().reset_index(name="num")

    df_a["status"] = df_a["status"].apply(str)

    df_b = df_a.pivot(index=["userId", "week_slice"],
                      columns="status", values="num").reset_index()
