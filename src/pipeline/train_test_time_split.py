...
def create_1_week_X_y_df(df: pd.DataFrame):
    ...
    for week_slice, (t0, t1, t2, slice_label) in enumerate(train_test_bins_1_week_window, start=1):
...
def create_2_week_X_y_df(df: pd.DataFrame):
    ...
    for week_slice, (t0, t1, t2, slice_label) in enumerate(train_test_bins_2_week_window, start=1):
...
def create_3_week_X_y_df(df: pd.DataFrame):
    ...
    for week_slice, (t0, t1, t2, slice_label) in enumerate(train_test_bins_3_week_window, start=1):
